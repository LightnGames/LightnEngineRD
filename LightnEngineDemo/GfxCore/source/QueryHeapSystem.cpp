...
void QueryHeapSystem::requestTimeStamp(CommandList* commandList, u32 frameIndex) {
	...
	MemoryRange range = { queryFrameOffset, queryFrameOffset + GPU_TIME_STAMP_COUNT_MAX };
	u64* mapedTimeStampPtr = _timeStampBuffer.map<u64>(&range);
	memcpy(_currentTimeStamps, mapedTimeStampPtr + queryFrameOffset, sizeof(u64) * GPU_TIME_STAMP_COUNT_MAX);
