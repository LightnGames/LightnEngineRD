...
void CommandList::setRenderTargets(u32 count, CpuDescriptorHandle rtvHandles, CpuDescriptorHandle* dsvHandle) {
	...
	D3D12_CPU_DESCRIPTOR_HANDLE rtvHandleD3D12 = toD3d12(rtvHandles);
	_commandList->OMSetRenderTargets(count, &rtvHandleD3D12, TRUE, dsvPtr);
}
...
void CommandList::setComputeRoot32BitConstants(u32 rootParameterIndex, u32 num32BitValuesToSet, const void* srcData, u32 destOffsetIn32BitValues) {
	_commandList->SetComputeRoot32BitConstants(rootParameterIndex, num32BitValuesToSet, srcData, destOffsetIn32BitValues);
}
...
void PipelineState::iniaitlize(const GraphicsPipelineStateDesc& desc) {
	...
	if (desc._dsvFormat != rhi::FORMAT_UNKNOWN) {
